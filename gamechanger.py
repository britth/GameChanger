...
def main_menu():
  ...
  Also prompts for which mode user wants to use
  '''
  ...
  while mode not in ('A', 'a', 'B', 'b'):
    user = raw_input("What mode do you want to explore? Type A to view updates in real time. Type B to view a graph of tweet volume for a particular game: ")
    if user in ('A', 'a'):
...
def print_sched(searchingFor, timestamp, matchups, schedule, tv, current_sched):
  '''
  Used to print schedule if there has been a change in lineup 
  (i.e. a new game has started or a game has ended)
  '''
